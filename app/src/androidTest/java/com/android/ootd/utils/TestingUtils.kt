...
fun checkItemAppearsInPost(composeTestRule: ComposeContentTestRule) {
  // 1. Open the post (clicks the card gets taken to PostViewScreen)
  clickWithWait(composeTestRule, OutfitPostCardTestTags.OUTFIT_POST_CARD)

  // Assert we are on PostViewScreen
  verifyElementAppearsWithTimer(composeTestRule, PostViewTestTags.SCREEN)

  // Assert items section exists
  verifyElementAppearsWithTimer(composeTestRule, PostViewTestTags.ITEMS_SECTION)

  // 4. Navigate back to feed
  clickWithWait(composeTestRule, PostViewTestTags.BACK_BUTTON)
}
