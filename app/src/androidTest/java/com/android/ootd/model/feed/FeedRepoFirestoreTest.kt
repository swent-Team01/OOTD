...
class FeedRepoFirestoreTest {
    ...
    fun getFeed_onNetworkFailure_returnsEmptyList() = runBlocking {
        val invalidDb = firestoreForApp(appName = "feed-repo-unreachable", host = "10.0.2.2", port = 6553)
    ...
    fun addPost_onNetworkFailure_isLocallyAcknowledged_andRepoReadIsEmpty() = runBlocking {
        ...
        val badDbReader = firestoreForApp(appName = "feed-repo-local-read", host = "10.0.2.2", port = 6553)
    ...
    fun hasPostedToday_defaultFalse() = runBlocking {
        assertEquals(false, repo.hasPostedToday())
    }
    ...
    fun checkPostData_returnsNullForInvalid_andSameForValid() = runBlocking {
        ...
        val method = FeedRepositoryFirestore::class.java.getDeclaredMethod("checkPostData", OutfitPost::class.java)
    ...
    fun transformPostDocument_handlesValidAndInvalid() = runBlocking {
        ...
        val invalidSnapshot = db.collection(POSTS_COLLECTION_PATH).document("transform-bad").get().await()

        val method =
            FeedRepositoryFirestore::class.java.getDeclaredMethod("transformPostDocument", DocumentSnapshot::class.java)
        method.isAccessible = true
    ...
    private fun firestoreForApp(appName: String, host: String, port: Int): FirebaseFirestore {
        ...
        val defaultApp = FirebaseApp.getApps(context).firstOrNull() ?: FirebaseApp.initializeApp(context)!!
        val app =
            try {
